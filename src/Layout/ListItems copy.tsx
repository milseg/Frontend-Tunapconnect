  ...
  return (
    <React.Fragment>
      {menuListCompanyId.map((menu, index) => {
        return menu.path === '/upload'
          ? routeActual !== '/service-schedule' && (
              <React.Fragment key={index}>
                ...
                <Collapse
                  in={uploadOpen && registerOpen}
                  timeout="auto"
                  unmountOnExit
                >
                  <List component="div" disablePadding>
                    <Link
                      href={`${menu.href}?status=toyolex`}
                      style={{ textDecoration: 'none' }}
                      onClick={handleListReload}
                    >
                      <ListItemButton
                        sx={{ pl: 4, marginLeft: '70px', width: 'fit-content' }}
                      >
                        <ListItemText primary="Upload Toyolex" />
                      </ListItemButton>
                    </Link>
                    <Link
                      href={`${menu.href}?status=umuarama`}
                      style={{ textDecoration: 'none' }}
                      onClick={handleListReload}
                    >
                      <ListItemButton
